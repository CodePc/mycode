...
//BusReservation br = new BusReservation();
        ...
        if (ticketsBooked == true) {
            System.out.println("CONGRATS" + Thread.currentThread().getName() + "..The number of seats " +
                    "requested(" + pt.getSeatsNeeded() + ")are BOOKED");
        } else {
            System.out.println("Sorry Mr." + Thread.currentThread().getName() + "..The number of seats " +
                    "requested(" + pt.getSeatsNeeded() + ")are not available");
        }
    }
